...
function ThemesPanel({ dashboard, reload, showMessage, confirmAction }: { dashboard: Dashboard; reload: () => Promise<void>; showMessage: (value: string) => void; confirmAction: ConfirmAction }) {
  ...
  async function applyAction(id: string, work: (restart: boolean) => Promise<unknown>, message: string) {
    await action(id, async () => {
      const status = await api.desktopStatus();
      const restart = !status.connected;
      if (restart && !await confirmAction("需要重启 Codex Desktop", "换肤连接尚未建立或已经失效，需要用本地调试端口启动 Codex。请先等待运行中的任务结束并保存输入。", false)) throw new Error("已取消应用主题");
      await work(restart);
    }, message);
  }
  ...
  return <>
    ...
    <section><div className="section-title"><h2>主题市场</h2>{loaded && <span>{page.total}</span>}<button className="section-action" disabled={busy === "market"} onClick={() => void refresh()}>{busy === "market" ? "加载中…" : "刷新市场"}</button></div>
      ...
      {!loaded ? <p className="empty">按需刷新市场；列表每页最多渲染 24 个主题。</p> : <div className="theme-grid">{page.items.map(item => <ThemeCard key={`${item.sourceName}-${item.id}`} item={item} installed={installed.has(item.id)} applied={dashboard.themeState.appliedThemeId === item.id} busy={busy === item.id} onInstall={() => { void applyAction(item.id, restart => api.installAndApplyTheme(item.id, item.sourceName, restart), "主题已安装并应用"); }} onApply={() => { void applyAction(item.id, restart => api.applyTheme(item.id, restart), "主题已应用"); }} onRemove={() => { void confirmAction("卸载这个主题？", item.name, true).then(ok => { if (ok) void action(item.id, () => api.uninstallTheme(item.id), "主题已卸载"); }); }} />)}</div>}
