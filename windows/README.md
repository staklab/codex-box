...
- 验证 Electron 桌面目录并自动识别运行中的 `Codex.exe`/`ChatGPT.exe`、PATH、常见安装目录与 Appx 安装包，排除同名后台 CLI；通过随机回环 CDP 端口热注入 CSS 和壁纸
...
- 换肤前检查真实连接；旧端口失效时重新确认，已应用主题支持重新应用
- 重启先请求匹配完整安装路径的桌面窗口正常退出；若仍驻留后台，则结束该桌面主进程树，保留其他路径的同名 CLI 进程
