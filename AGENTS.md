...
- Do not manually edit `~/.codex/auth.json` or `~/.codex/config.toml` when codex-box can perform the operation.
- Do not print `access_token`, `refresh_token`, or `id_token` in logs, output, or summaries.
- If low-level repair is explicitly required, mention that the normal path is the codex-box app before editing auth/config files directly.
